...
async function checkGTM() {
  log("\n=== Testing Google Tag Manager Integration ===\n", "cyan")
  ...
  // Check GTM ID format
  if (!gtmId) {
    log("✘ GTM: ID missing", "red")
    allValid = false
  } else {
    const gtmIdPattern = /^GTM-[A-Z0-9]+$/
    if (!gtmIdPattern.test(gtmId)) {
      log("✘ GTM: Invalid ID format", "red")
      log(`  Expected format: GTM-XXXXXXX, got: ${gtmId}`, "red")
      allValid = false
    } else {
...
async function runAudit() {
  ...
  const gtmCheck = await checkGTM()

  log("\n=== Pre-Development Audit Summary ===\n", "cyan")
  ...
  log(`Google Tag Manager: ${gtmCheck ? "PASS ✓" : "FAIL ✘"}`, gtmCheck ? "green" : "red")

  const overallResult = envCheck && recaptchaCheck && resendCheck && gtmCheck

  log("\nOverall Result: " + (overallResult ? "PASS ✓" : "FAIL ✘"), overallResult ? "green" : "red")

  if (overallResult) {
    log("\nAll integrations are properly configured. Ready for development!", "green")
  } else {
    log("\nSome issues need to be fixed before proceeding. See details above.", "red")
  }
}
